...
| 3.0.0 | 2 | sol, fable | B5 | SECURITY: VendorIdentityLoader read ordinary vendor credential stores (~/.codex/~/.claude token-bearing files, INV-067) and misattributed a native-login row's identity; app-side read removed (INV-002), email/plan metadata deferred to 3.1 | fixed | wave-2 batch fix; docs/FEATURES.md macos/accounts |
