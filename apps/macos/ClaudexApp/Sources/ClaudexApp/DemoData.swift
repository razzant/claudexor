...
enum DemoData {
    ...
    static let raceTask = TaskRun(
        ...
        activity: [
            ...
            ActivityEvent(.thinking, harness: .codex, "Mapping repo config keys to TaskContract.tests.commands", detail: "Found gates in claudex.config.json: build, test, typecheck.", at: ago(40)),
        ...
        findings: [
            Finding(id: "f1", severity: .major, category: "Correctness", title: "CLI --gate flag not threaded into contract", detail: "codex candidate resolves gates from config only; CLI-provided gates are dropped before the contract is built, so `claudex race --gate \"pnpm test\"` is silently ignored.", reviewer: .cursor, routeProof: .verified, evidenceFile: "packages/cli/src/cli.ts", evidenceLine: 412, accepted: true, taskTitle: "Wire config → deterministic gates"),
            Finding(id: "f2", severity: .minor, category: "Tests", title: "No test for empty gate list staying vacuous-false", detail: "Add a regression asserting gatesPassed([]) does not report success once gates are required.", reviewer: .cursor, routeProof: .verified, evidenceFile: "packages/review/src/gates.test.ts", evidenceLine: nil, accepted: nil, taskTitle: "Wire config → deterministic gates"),
            Finding(id: "f3", severity: .nit, category: "Style", title: "Prefer typed GateSpec over string[]", detail: "Minor: a GateSpec record (name+command+required) would read better in artifacts.", reviewer: .codex, routeProof: .unverified, evidenceFile: nil, evidenceLine: nil, accepted: nil, taskTitle: "Wire config → deterministic gates"),
        ],
        diff: demoDiff
    )
    ...
    static let planTask = TaskRun(
        ...
        prompt: "Plan a notarized .app + DMG with Developer ID, hardened runtime (no sandbox), and a LaunchAgent for claudexd.",
        mode: .plan,
        status: .running,
        project: "claudex",
        specTitle: nil,
        ...
        activity: [
            ...
            ActivityEvent(.thinking, harness: .claude, "LaunchAgent vs SMAppService for claudexd lifecycle", at: ago(4)),
        ],
        candidates: [],
        findings: [],
        diff: []
    ...
    static let blockedTask = TaskRun(
        id: "run-44ab7e",
        title: "Swap Ouroboros claude_code.py for Claudex",
        prompt: "Replace Ouroboros' claude_code.py with the Claudex embeddable substrate over JSON-RPC.",
        mode: .maxAttempts,
        status: .blocked,
        project: "ouroboros",
        ...
        plan: [
            PlanItem("Map claude_code.py surface to Claudex API", .done),
    ...
    static let projects: [Project] = [
        Project(id: "p1", name: "claudex", specs: [
    ...
    static let benchmarks: [BenchmarkRun] = [
        BenchmarkRun(id: "b1", suite: "SWE-bench Verified", instance: "astropy__astropy-12907", status: .succeeded, resolved: true, costUsd: 0.31),
        BenchmarkRun(id: "b2", suite: "SWE-bench Verified", instance: "django__django-11099", status: .running, resolved: nil, costUsd: 0.12),
        BenchmarkRun(id: "b3", suite: "Terminal-Bench 2.1", instance: "oracle-reward-check", status: .succeeded, resolved: true, costUsd: 0.04),
        BenchmarkRun(id: "b4", suite: "Terminal-Bench 2.1", instance: "claudex-arm-node22", status: .failed, resolved: false, costUsd: 0.00),
    ]
