    ...
    private var notWiredCard: some View {
        Panel {
            VStack(alignment: .leading, spacing: Theme.Spacing.md) {
                ...
                Text("The hierarchical spec interview (claudexor spec) runs in the CLI today. The GUI flow is previewable with Sample data (Settings → Show sample data) and will drive a real, frozen SpecPack once wired over the control-api.")
                    .font(.callout).foregroundStyle(.secondary)
            }
        }
    }
