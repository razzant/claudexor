...
enum DemoData {
    ...
    static let raceTask = TaskRun(
        ...
        findings: [
            Finding(id: "f1", severity: .major, category: "Correctness", title: "CLI --gate flag not threaded into contract", detail: "codex candidate resolves gates from config only; CLI-provided gates are dropped before the contract is built, so `claudexor best-of --gate \"pnpm test\"` is silently ignored.", reviewer: .cursor, routeProof: .verified, evidenceFile: "packages/cli/src/cli.ts", evidenceLine: 412, status: .accepted, taskTitle: "Wire config → deterministic gates"),
