...
const check = (phase, name, ok, detail = {}) => {
  ...
  process.stdout.write(`${ok ? "PASS" : "FAIL"}  ${phase.padEnd(8)} ${name.padEnd(52)} ${JSON.stringify(detail).slice(0, 160)}\n`);
};
...
writeFileSync(join(fixtureRepo, "math.js"), "export function add(a, b) {\n  return a - b; // bug\n}\n");
spawnSync("git", ["init", "-q"], { cwd: fixtureRepo });
spawnSync("git", ["-c", "user.email=t@t", "-c", "user.name=t", "add", "-A"], { cwd: fixtureRepo });
spawnSync("git", ["-c", "user.email=t@t", "-c", "user.name=t", "commit", "-qm", "init"], { cwd: fixtureRepo });
const scratchEnv = { ...process.env, HOME: scratchHome, CLAUDEXOR_CONFIG_DIR: join(scratch, "config") };

function runCli(args, env, cwd = scratch) {
  const out = spawnSync(process.execPath, [cli, ...args], { env, cwd, encoding: "utf8", timeout: 180_000 });
  let json = null;
  try {
...
function jsonRpc(command, args, env, cwd) {
  ...
  rl.on("line", (l) => { if (l.trim()) { try { messages.push(JSON.parse(l)); } catch { /* noise */ } } });
  return {
    ...
    close: async () => { child.stdin.end(); await new Promise((r) => setTimeout(r, 200)); child.kill(); },
    stderrText: () => stderr,
...
async function phaseA() {
  ...
  check("A", "install cursor (scratch HOME)", install.code === 0 && install.json?.ok === true, { state: install.json?.results?.[0]?.state });
  const artifacts = [
  ...
  const allOwned = artifacts.every((a) => existsSync(join(pluginRoot, a)) && readFileSync(join(pluginRoot, a), "utf8").includes("claudexor:managed"));
  ...
  const mcpAction = (doctor.json?.results?.[0]?.actions ?? []).some((a) => a.includes("MCP initialize/tools-list self-test passed"));
  ...
  mkdirSync(join(collisionHome, ".cursor", "plugins", "local", "claudexor", "skills", "claudexor"), { recursive: true });
  writeFileSync(join(collisionHome, ".cursor", "plugins", "local", "claudexor", "skills", "claudexor", "SKILL.md"), "user file\n");
  const blocked = runCli(["plugin", "install", "cursor", "--force", "--json"], { ...scratchEnv, HOME: collisionHome, CLAUDEXOR_CONFIG_DIR: join(scratch, "config-collision") });
  check("A", "unowned SKILL.md blocks install (even --force)", blocked.json?.ok === false && /not Claudexor-owned/.test(blocked.stdout + blocked.stderr), {});
}
...
async function phaseC() {
  ...
  const mcpJsonPath = existsSync(realMcpJson) ? realMcpJson : join(scratchHome, ".cursor", "plugins", "local", "claudexor", "mcp.json");
  const cfg = JSON.parse(readFileSync(mcpJsonPath, "utf8"));
  const server = cfg.mcpServers?.claudexor;
  check("C", "registered command parses", Boolean(server?.command && Array.isArray(server?.args)), { source: mcpJsonPath === realMcpJson ? "real" : "scratch" });
  const srv = jsonRpc(server.command, server.args, { ...process.env, ...(server.env ?? {}), CLAUDEXOR_CONFIG_DIR: join(scratch, "config-c") }, fixtureRepo);
  try {
    srv.send({ jsonrpc: "2.0", id: 0, method: "initialize", params: { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "cursor-itest", version: "1.0" } } });
    const init = await srv.waitFor((m) => m.id === 0, 20_000);
    check("C", "initialize negotiates 2025-06-18", init?.result?.protocolVersion === "2025-06-18", { serverInfo: init?.result?.serverInfo });
    ...
    srv.send({ jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "claudexor_run", arguments: { prompt: "x", repoPath: "relative/path" } } });
    const invalid = await srv.waitFor((m) => m.id === 2, 15_000);
    check("C", "invalid args -> isError result", invalid?.result?.isError === true, { head: String(invalid?.result?.content?.[0]?.text ?? "").slice(0, 80) });
    // Live fake ask through the registered command + ping DURING it.
    srv.send({ jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "claudexor_ask", arguments: { prompt: "what is 2+2?", repoPath: fixtureRepo, harness: "fake-success" } } });
    ...
    check("C", "ask returns text + runId trailer", !ask?.result?.isError && /runId: \S+/.test(text), { head: text.slice(0, 60) });
  } finally {
    await srv.close();
  }
}
...
async function phaseD() {
  ...
  try {
    ...
    srv.send({
      ...
      id: 1,
      method: "tools/call",
      params: { name: "claudexor_run", arguments: { prompt: "fix add() so it adds", repoPath: fixtureRepo, harness: "fake-implement", tests: ['node -e "process.exit(0)"'] } },
    });
    ...
    check("D", "inspect resolves the MCP-started run", inspect.code === 0 && inspect.stdout.includes(runId ?? "@"), {});
    ...
    check("D", "run artifacts live under the project repo", existsSync(join(runDir, "context", "task.yaml")), { runDir });
...
async function failureModes() {
  ...
  try {
    ...
    await srv.waitFor((m) => m.id === 0, 20_000);
    srv.send({ jsonrpc: "2.0", method: "notifications/initialized" });
    srv.send({ jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "claudexor_ask", arguments: { prompt: "x", repoPath: fixtureRepo, harness: "no-such-harness" } } });
    const bad = await srv.waitFor((m) => m.id === 1, 60_000);
    const text = String(bad?.result?.content?.[0]?.text ?? "");
    check("F", "unknown harness fails LOUD in the result", /unknown harness/.test(text), { head: text.slice(0, 80) });
  } finally {
    await srv.close();
  }
  // Mode 2: stale cliPath in mcp.json -> the registered command fails to boot (host-visible).
  const staleCmd = jsonRpc(process.execPath, [join(scratch, "nonexistent-cli.js"), "mcp", "serve"], scratchEnv, scratch);
  const exitCode = await Promise.race([staleCmd.exited, new Promise((r) => setTimeout(() => r("timeout"), 10_000))]);
  check("F", "stale cliPath fails to boot (nonzero exit)", exitCode !== 0 && exitCode !== "timeout", { exitCode });
  // Mode 3: version-skew warning on stderr.
  const skew = jsonRpc(process.execPath, [cli, "mcp", "serve"], { ...scratchEnv, CLAUDEXOR_PLUGIN_VERSION: "0.0.1", CLAUDEXOR_CONFIG_DIR: join(scratch, "config-f3") }, scratch);
  skew.send({ jsonrpc: "2.0", id: 0, method: "initialize", params: { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "x", version: "1" } } });
  await skew.waitFor((m) => m.id === 0, 15_000);
  await skew.close();
  check("F", "version-skew warning on stderr", skew.stderrText().includes("plugin artifacts are version 0.0.1"), {});
}
...
process.stdout.write("Manual phases B (Cursor discovery after reload) and E (agent-in-the-loop) live in docs/CHECKLISTS.md.\n");
