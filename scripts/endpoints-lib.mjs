...
export async function endpointDetails() {
  ...
  const details = OPERATION_CATALOG.operations.map((op) => ({
    ...
    // Non-body (query/header) parameters — the body shape is requestSchema (QA-055).
    ...
    JSON.stringify(
      {
        ...
        endpoints: details.map((d) => ({
          ...
          // Query/header parameters (request BODY shape is request_schema).
