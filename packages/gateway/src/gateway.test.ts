...
describe("HarnessGateway auth readiness projection", () => {
  it("projects doctor auth_sources without inferring readiness from the manifest", async () => {
    const adapter = {
      ...
      discover: async () => HarnessManifest.parse({
        id: "fake-auth-projection",
    ...
    const rows = await new HarnessGateway(new Map([[adapter.id, adapter]])).statusAll({ cwd: "/repo" });
  ...
  it("targets one harness/source without calling discover or unrelated adapters", async () => {
    ...
    const target = {
      ...
      discover: async () => { throw new Error("discover must not run"); },
      doctor: async (spec) => {
    ...
    await expect(gateway.probeAuthSource(target.id, "native_session", {
      cwd: "/repo",
      fresh: true,
    })).resolves.toEqual({
