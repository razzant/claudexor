...
export const Severity = z
  .enum(["BLOCK", "FIX_FIRST", "WARN", "NIT", "OUT_OF_SCOPE", "INSUFFICIENT_EVIDENCE", "NEEDS_HUMAN"])
  .describe(
...
export const FileEvidence = z
  .object({
    ...
    lines: z.string().nullable().default(null).describe('Line range in the file (e.g. "10-42"); null when the whole file is cited.'),
  })
...
export const FindingEvidence = z
  .object({
    ...
    commands: z.array(CommandEvidence).default([]).describe("Commands whose output supports the finding."),
    logs: z.array(z.object({ path: z.string().describe("Log file path.") })).default([]).describe("Log files cited as evidence."),
  })
...
export const ReviewFinding = z
  .object({
    ...
    linked_acceptance_criteria: z.array(Id).default([]).describe("Ids of acceptance criteria this finding relates to."),
    evidence: FindingEvidence.default({}),
    proposed_fix: z.string().nullable().default(null).describe("Reviewer-proposed fix; null when none was offered."),
    reviewer: z
      .object({
        harness_id: Id.describe("Harness that produced the review."),
        requested_model: z.string().nullable().default(null).describe("Model requested for the reviewer route."),
        requested_effort: z.string().nullable().default(null).describe("Reasoning effort requested for the reviewer route."),
        observed_model: z.string().nullable().default(null).describe("Model the harness actually reported using."),
    ...
    revalidation_note: z.string().optional().describe("Note recorded when the finding was revalidated against a changed tree."),
  })
