...
export const ControlCandidate = z
  .object({
    ...
    label: z.string().nullable().default(null).describe("Human-readable candidate label, when set."),
    costUsd: z.number().default(0).describe("Spend attributed to the candidate, in USD."),
    costEstimated: z.boolean().default(false).describe("True when the cost is token-derived rather than natively reported."),
    errored: z.boolean().default(false).describe("True when the candidate's attempt errored."),
    gatesPassed: z.number().int().nonnegative().default(0).describe("Number of deterministic gates that passed."),
    gatesTotal: z.number().int().nonnegative().default(0).describe("Total deterministic gates run for the candidate."),
    ...
    reviewVerified: z.boolean().default(false).describe("Whether the candidate's review route was verified."),
    finalReviewClean: z.boolean().nullable().default(null).describe("Whether the final review was clean; null when no final review ran."),
...
export const ControlRunDetail = z
  .object({
    ...
    artifacts: z.array(ControlArtifactInfo).default([]).describe("Artifacts recorded in the run tree."),
    primaryOutput: ControlPrimaryOutput.nullable().default(null).describe("The run's primary user-facing output; null while pending."),
    timeline: z.array(ControlTimelineEvent).default([]).describe("Projected timeline of run events."),
    budget: ControlBudgetSnapshot.default({}),
    finalSummary: z.string().nullable().default(null).describe("Final summary text; null when the run has none."),
    decision: DecisionRecord.nullable().default(null).describe("The arbitration decision; null before arbitration."),
    ...
    operatorDecision: z
      ...
      .nullable()
      .default(null)
      .describe("Persisted operator unblock decision (accept_risk/override), hash-bound; null when none was made."),
    workProduct: WorkProduct.nullable().default(null).describe("The run's work product; null when none was produced."),
    ...
    applyEligibility: ApplyEligibility.nullable()
      .default(null)
      .describe("Derived apply-gate verdict (single producer: the delivery gate); null when the run has no patch artifact."),
    reviewFindings: z.array(ReviewFinding).default([]).describe("Review findings recorded for the run."),
    pendingInteractions: z.array(ControlPendingInteraction).default([]).describe("Interactive questions currently awaiting answers."),
    ...
    orchestrate: OrchestratePlanProgress.nullable()
      .default(null)
      .describe("Typed executor progress for an orchestrate run; null for non-orchestrate runs or suggest autonomy."),
    ...
    candidates: z
      ...
      .default([])
      .describe("Per-candidate evidence cards for every envelope-producing mode; empty only when no attempt artifacts exist."),
    ...
    planProgress: z
      ...
      .nullable()
      .default(null)
      .describe("Live plan checklist from the last plan.progress event; null when the run never emitted one."),
    failure: RunFailure.nullable().default(null).describe("Typed failure info; null unless the run failed."),
  })
  .describe("Full run detail snapshot served by GET /runs/:id: summary, artifacts, timeline, decision, findings, and progress.");
