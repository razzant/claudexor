...
function skillText(host: PluginHost, runtime: RuntimePaths): string {
  return [
    ...
    "- MCP tools `claudexor_thread_create` + `claudexor_thread_turn` for durable multi-turn work.",
    ...
    "Use `credentialProfileId` for a strict account pin. For multi-turn supervision, create the thread once, enqueue each follow-up with `claudexor_thread_turn`, then follow its returned `runId` with the ordinary status/result tools. If the turn returns only a queued `jobId`, use `claudexor_runs` to recover its `runId` after binding.",
    "",
    READINESS_GUIDANCE,
