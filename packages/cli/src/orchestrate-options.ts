import { OrchestrateAutonomy, type OrchestrateAutonomy as OrchestrateAutonomyValue } from "@claudexor/schema";
...
export function parseAutonomy(value: string | undefined): OrchestrateAutonomyValue | undefined {
  ...
  if (!parsed.success) throw new Error(`invalid --autonomy '${value}' (expected suggest|auto_safe|auto_full)`);
  return parsed.data;
}
