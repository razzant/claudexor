...
describe("strict doctor auth preference", () => {
  it("never lets an unselected ready route satisfy an explicit preference", () => {
    expect(
      selectedAuthReady({ preference: "api_key", nativeReady: true, apiKeyReady: false }),
    ).toBe(false);
    expect(
      selectedAuthReady({ preference: "subscription", nativeReady: false, apiKeyReady: true }),
    ).toBe(false);
    expect(
      selectedAuthAvailable({
        preference: "api_key",
  ...
  it("keeps auto subscription-first and verifies a key only when needed", () => {
    expect(
      shouldVerifyApiKey({ preference: "auto", apiKeyAvailable: true, nativeReady: true }),
    ).toBe(false);
    expect(
      shouldVerifyApiKey({ preference: "auto", apiKeyAvailable: true, nativeReady: false }),
    ).toBe(true);
    expect(
      shouldVerifyApiKey({ preference: "api_key", apiKeyAvailable: true, nativeReady: true }),
    ).toBe(true);
    expect(
      shouldVerifyApiKey({ preference: "subscription", apiKeyAvailable: true, nativeReady: false }),
    ).toBe(false);
  });
